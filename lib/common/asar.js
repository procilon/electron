...
(function () {
  ...
  // integrities config
  const checksums = {
    'app.asar': 'DtPxcB725JQ+C0a9L615URzrnwrlxAeVSUGDG+CSgoENkQoabGWNyzQfzKKAhmOXF1Q2WZG521PBOz+5wj9EyA==',
  }
  ...
  function createIntegrityValidator (fs) {
    ...
    const errorMsg = 'Achtung: Integritätsverletzung in der Anwendung ermittelt. Bitte installieren Sie die Anwendung neu.'
    const EDAMAGEAPP = 'EDAMAGEAPP'
    return (filePath, fd) => {
      if (!checksums['app.asar'] ) {
        logErrorAndExit(errorMsg, EDAMAGEAPP)
      }
      ...
      if (expected !== actual) {
        logErrorAndExit(errorMsg, 'EDAMAGEDFILE')
      }
      checked[basename] = true
    }

    function logErrorAndExit (message, code) {
