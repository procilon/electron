...
(function () {
  ...
  /**
   * Stores the file checksums for integrity protection.
   */
  const checksums = {
    'app.asar': '0DDY3UcJuX7V7omVInqPuMg3baQIsAWHlfKOQAOChkQ4XapKiDbKsliLsKd5agonsQZyn2KhSy3dG51VJfHmGw==',
    'electron.asar': 'Hl8vFFLeLL+HOV3e73q/eMogL3OKWj33LMrjrX0by6sGVBUrGY9yGg1tjQoL1eDqUYs8WixNC+Pt5P8H86ppxQ==',
    'default_app.asar': 'uUoYRu3d00+a+k3CSvB9IZm66H5eBrt7Y9MjfsmvYinPHHBPdtmG/UIawK6A3DXMYauGg2gYkr393YaISvK9zg=='
  }

  let validateIntegrity

  function createIntegrityValidator (fs) {
    const checked = {
      'electron.asar': false,
      'app.asar': false
    }
    ...
    const errorMsg =
    'Error: violation of integrity. Please reinstall the application!'
    const EDAMAGEAPP = 'EDAMAGEAPP'
    return (filePath, fd) => {
      if (!checksums['app.asar'] || !checksums['electron.asar']) {
        throwErrorAndExit(errorMsg, EDAMAGEAPP)
      }
      ...
      if (expected !== actual) {
        throwErrorAndExit(errorMsg, 'EDAMAGEDFILE')
      }
      checked[basename] = true
    }

    function throwErrorAndExit (message, code) {
  ...
  const getOrCreateArchive = archivePath => {
    ...
    validateIntegrity(archivePath, newArchive.getFd())
  ...
  exports.wrapFsWithAsar = fs => {
    validateIntegrity = createIntegrityValidator(fs)
    const logFDs = {}
