(function () {
   ...
   *
   * Required Interfaces:
   * () -> FileDescriptor e.g. from a command like `const fd = fs.openSync('temp.txt', 'r');`
   *
   * @param {string} filePath path of the requested file
   * @param {() => FileDescriptor} fileFd (archive) object to get a FileDescripter via `getFd`, to calculate the actual checksum (sha512). Optional, if not set, we'll call openSync and closeSync by ourselfs. CloseSync is only called of no FD is given.
   */
   ...
   * @param {FileSystem} fs 
   */
  const createIntegrityValidator = fs => {
    ...
    const EDAMAGEDAPP = 'EDAMAGEDAPP'
    return (filePath, fd) => {
      if (!checksums['app.asar'] || !checksums['electron.asar']) {
        throwErrorAndExit(errorMsg, EDAMAGEDAPP)
      }
  ...
  const getOrCreateArchive = function (p) {
    ...
    validateIntegrity(p, archive.getFd())
